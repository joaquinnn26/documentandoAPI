...
export const findCart = async (req, res, next) => {
    try{
        ...
        if (!cart) {
                customError.createError(errorsName.CART_NOT_FOUND,errorsMessage.CART_NOT_FOUND,400)
            }
...
export const addProductToCart =  async (req, res,next) => {
    ...
    console.log(user)
    if (!cid || !pid ) {
        customError.createError(errorsName.DATA_NOT_RECEIVED,errorsMessage.DATA_NOT_RECEIVED,400)
    }
    try {   
        const product= await productsService.findProdById(pid)
        console.log(product)
        if (user._id==product.owner) {
            customError.createError(errorsName.YOU_CREATED_PRODUCT,errorsMessage.YOU_CREATED_PRODUCT,404)
        }
